...
## [Unreleased]
### Changed
- **Dependency**: PymordialBlue now depends on `pymordialdroid` for shared Android device implementations.
- **Removed**: Duplicated `PymordialAndroidApp`, `AdbDevice`, `AndroidUiDevice`, and `TesseractDevice` modules. PymordialBlue now reuses these from `pymordialdroid` directly.
- **Exports**: Package exports and docs updated to canonical `pymordialdroid` class names. `PymordialBluestacksDevice` remains the only BlueStacks-specific implementation in this repo.
...
- **App Model**: `AndroidApp` for lifecycle management.
- **Devices**:
    - `AdbDevice`: ADB shell, input, and H.264 streaming.
    - `PymordialBluestacksDevice`: Process management for BlueStacks.
    - `AndroidUiDevice`: Template matching and pixel verification.
    - `TesseractDevice`: Tesseract OCR integration.
